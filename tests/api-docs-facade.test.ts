...
describe('API docs facade access', () => {
  it('maps a parameterized path to ergonomic access', () => {
    expect(apiDocsFacadeAccess('/applicant/Exame/all/{id}', 'get')).toBe('apiDocs.applicant.exame.all.params.id.get');
  });
  it('camel-cases compound segments and supports a custom root', () => {
    expect(apiDocsFacadeAccess('/user-profiles/{user_id}', 'delete', 'client')).toBe('client.userProfiles.params.userId.delete');
  });
