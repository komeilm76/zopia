...
| R-655 | 🧱 **Components** | the manifest **always** lists schema components (name, full `schema`, `file: <path> \| null`) and preserves other OpenAPI component sections in `componentsOverlay`; Swagger reusable `parameters` and `responses` are retained separately. `file` set (components mode): the component file is imported and converted — developer edits win. `file: null` (default mode): the manifest `schema` is re-emitted verbatim. Either way, use sites become `$ref`s via the per-API ref pointers (R-752). |
