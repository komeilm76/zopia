...
| T-7 | **`index.ts` files are TypeScript** and fill **all practical content** of the endpoint (method, path, summary, description, tags, auth, content types, request, response, examples) using **`makeApiConfig()` from km-api** (the package's make function) | [API docs format → `index.ts` contract](07-api-docs.md#-the-indexts-contract) | ✅ 🚧 |
