...
- 🧩 `useComponentAsReference` now imports exact endpoint component schema references through the component barrel; nested/cyclic imports remain pending.
...
- 📦 Added default `.zopia-manifest.json` generation with source hash, security metadata, component schemas, API file mappings, per-operation `$ref` metadata, and response-header overlays.
