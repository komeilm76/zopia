...
function renderComponent(name: string, schema: unknown, source: OpenApiDocument): string {
  ...
  if (schema && typeof schema === 'object' && !Array.isArray(schema) && (schema as any).type === 'object' && (schema as any).properties && typeof (schema as any).properties === 'object') {
    ...
    const fields = Object.entries(properties).map(([key, value]) => { const target = componentTarget(value && typeof value === 'object' ? (value as any).$ref : undefined); const ref = target === undefined ? undefined : `${exportName(target)}Schema`; if (ref) { imports.set(ref, target!); return `${JSON.stringify(key)}: ${ref}${Array.isArray((schema as any).required) && (schema as any).required.includes(key) ? '' : '.optional()'}`; } return `${JSON.stringify(key)}: ${schemaCode(value, `${name}${key}`)}${Array.isArray((schema as any).required) && (schema as any).required.includes(key) ? '' : '.optional()'}`; }).join(', ');
    const importLine = [...imports.entries()].sort(([a], [b]) => a.localeCompare(b)).filter(([ref]) => ref !== componentName).map(([ref, target]) => `import { ${ref} } from '../${target}/index';`).join('\\n');
