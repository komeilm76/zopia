...
function renderComponent(name: string, schema: unknown, source: OpenApiDocument): string {
  ...
  const directRef = componentExport(schema && typeof schema === 'object' ? (schema as any).$ref : undefined);
  if (directRef && directRef !== componentName) return `/** Generated by zopia — do not edit by hand. */\nimport { ${directRef} } from '../${directRef.replace(/Schema$/, '')}/index';\n\nexport const ${componentName} = ${directRef};\n\nexport default ${componentName};\n`;
  ...
  if (schema && typeof schema === 'object' && !Array.isArray(schema) && (schema as any).type === 'array' && (schema as any).items && componentExport((schema as any).items.$ref)) {
    const item = componentExport((schema as any).items.$ref)!;
    return `/** Generated by zopia — do not edit by hand. */\nimport { ${item} } from '../${item.replace(/Schema$/, '')}/index';\n\nexport const ${componentName} = z.array(${item});\n\nexport default ${componentName};\n`;
  }
  if (schema && typeof schema === 'object' && !Array.isArray(schema) && (schema as any).type === 'object' && (schema as any).properties && typeof (schema as any).properties === 'object') {
    ...
    const fields = Object.entries(properties).map(([key, value]) => { const ref = componentExport(value && typeof value === 'object' ? (value as any).$ref : undefined); if (ref) { imports.add(ref); return `${JSON.stringify(key)}: ${ref}${Array.isArray((schema as any).required) && (schema as any).required.includes(key) ? '' : '.optional()'}`; } return `${JSON.stringify(key)}: ${schemaCode(value, `${name}${key}`)}${Array.isArray((schema as any).required) && (schema as any).required.includes(key) ? '' : '.optional()'}`; }).join(', ');
    const importLine = [...imports].sort().filter((ref) => ref !== componentName).map((ref) => `import { ${ref} } from '../${ref.replace(/Schema$/, '')}/index';`).join('\\n');
