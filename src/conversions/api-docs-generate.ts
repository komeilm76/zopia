...
function renderComponent(name: string, schema: unknown, source: OpenApiDocument): string {
  ...
  if (schema && typeof schema === 'object' && !Array.isArray(schema) && (schema as any).type === 'object' && (schema as any).properties && typeof (schema as any).properties === 'object') {
    ...
    const fields = Object.entries(properties).map(([key, value]) => `${JSON.stringify(key)}: ${renderNestedSchema(value, `${name}${key}`, imports)}${required.has(key) ? '' : '.optional()'}`).join(', ');
    ...
    const expression = imports.has(componentName) ? `z.lazy(() => z.object({ ${fields} }))` : `z.object({ ${fields} })`;
