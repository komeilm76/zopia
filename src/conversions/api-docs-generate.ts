...
export async function generateApiDocsFiles(input: OpenApiDocument | string, options: GenerateApiDocsOptions): Promise<GeneratedApiDocsFile[]> {
  ...
  if (options.manifest !== false) {
    ...
    const manifest = {
      ...
      servers: source.servers ?? (source.basePath ? [source.basePath] : ['/']), ...(source.swagger === '2.0' ? { swaggerHost: source.host, swaggerSchemes: source.schemes } : {}), tags: source.tags ?? [], securitySchemes: source.components?.securitySchemes ?? source.securityDefinitions ?? {},
