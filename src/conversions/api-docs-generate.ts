...
function renderComponent(name: string, schema: unknown, source: OpenApiDocument): string {
  ...
  if (schema && typeof schema === 'object' && !Array.isArray(schema) && (schema as any).type === 'array' && (schema as any).items && componentExport((schema as any).items.$ref)) {
    const itemTarget = componentTarget((schema as any).items.$ref)!; const item = `${exportName(itemTarget)}Schema`;
    return `/** Generated by zopia — do not edit by hand. */\nimport { ${item} } from ${JSON.stringify(`../${itemTarget}/index`)};\n\nexport const ${componentName} = z.array(${item});\n\nexport default ${componentName};\n`;
  }
  if (schema && typeof schema === 'object' && !Array.isArray(schema) && (schema as any).type === 'object' && (schema as any).properties && typeof (schema as any).properties === 'object') {
    const properties = (schema as any).properties as Record<string, unknown>;
    const imports = new Map<string, string>();
    const fields = Object.entries(properties).map(([key, value]) => { const target = componentTarget(value && typeof value === 'object' ? (value as any).$ref : undefined); const arrayTarget = target === undefined && value && typeof value === 'object' && (value as any).type === 'array' ? componentTarget((value as any).items?.$ref) : undefined; const ref = target === undefined ? undefined : `${exportName(target)}Schema`; if (ref) { imports.set(ref, target!); return `${JSON.stringify(key)}: ${ref}${Array.isArray((schema as any).required) && (schema as any).required.includes(key) ? '' : '.optional()'}`; } if (arrayTarget) { const arrayRef = `${exportName(arrayTarget)}Schema`; imports.set(arrayRef, arrayTarget); return `${JSON.stringify(key)}: z.array(${arrayRef})${Array.isArray((schema as any).required) && (schema as any).required.includes(key) ? '' : '.optional()'}`; } return `${JSON.stringify(key)}: ${schemaCode(value, `${name}${key}`)}${Array.isArray((schema as any).required) && (schema as any).required.includes(key) ? '' : '.optional()'}`; }).join(', ');
    ...
    return `/** Generated by zopia — do not edit by hand. */\\nimport { z } from 'zod';\\n${importLine}${importLine ? '\\n' : ''}\\nexport const ${componentName} = z.object({ ${fields} });\\n\\nexport default ${componentName};\\n`;
  }
